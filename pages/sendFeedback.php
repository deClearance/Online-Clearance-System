...
if (!isset($_SESSION['id'])) {
    ...
    if (isset($_POST['send'])) {
        ...
        if (empty($desc) || empty($office)) {
            header("Location:./sendFeedback.php?message=Please Fill Empty Fields First Please!");
        } else {
            print_r($in_charge);
            ...
            if ($result) {
                header("Location:./sendFeedback.php?message=New Feddeback Successfully Sent!");
            } else {
                header("Location:./sendFeedback.php?message=Unable To Send Feedback!");
